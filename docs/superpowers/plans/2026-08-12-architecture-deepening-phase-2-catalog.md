...
- `tests/extract/config-video.test.ts` — retain the `gemini` fallback assertion from the catalog-backed map.
...
Move the provider imports currently assembled in `src/providers/all.ts` into `src/providers/catalog.ts`. Preserve the current order so registration and dashboard output remain stable. Resolve imported metadata by name, then define one ordered catalog literal containing each provider’s default config and fallback environment. This keeps identity, defaults, and fallback facts in one entry instead of three parallel maps.
...
const providerCatalog: readonly ProviderCatalogEntry[] = [
  ...
  catalogEntry("brave-llm", {
    enabled: true,
    budget: { mode: "hard", limit: 5, period: "month", unit: "usd", pool: "brave" },
    apiKey: "BRAVE_API_KEY",
  }, "BRAVE_API_KEY"),
  catalogEntry("context7", {
    enabled: true,
  ...
  catalogEntry("openai-web-search", {
    enabled: true,
    budget: { mode: "managed" },
    apiKey: "OPENAI_API_KEY",
  }, "OPENAI_API_KEY"),
  catalogEntry("parallel", {
    enabled: false,
    budget: { mode: "managed" },
    apiKey: "PARALLEL_API_KEY",
  }, "PARALLEL_API_KEY"),
  catalogEntry("perplexity", {
    enabled: true,
    budget: { mode: "managed" },
  ...
  catalogEntry("tinyfish", {
    enabled: true,
    budget: { mode: "unlimited" },
    apiKey: "TINYFISH_API_KEY",
  }, "TINYFISH_API_KEY"),
  catalogEntry("websearchapi", {
    enabled: false,
...
Derive all consumer projections from `providerCatalog` and keep the service fallback in the same environment map:

```ts
export { providerCatalog };

export const allProviders: ProviderMeta[] = providerCatalog.map(({ meta }) => meta);
...
export const fallbackEnvMap: Record<string, string> = Object.fromEntries([
  ...
  ...serviceEnvironmentEntries.map(({ name, fallbackEnv }) => [name, fallbackEnv] as const),
]);
```
...
if (catalogNames.size !== providerCatalog.length) throw new Error("Duplicate provider catalog name");
...
for (const meta of providerMetas) {
  if (!catalogNames.has(meta.name)) throw new Error(`Provider ${meta.name} is missing from catalog`);
}
```
...
describe("provider catalog", () => {
  ...
  it("has a valid default config for every provider", () => {
    for (const { meta, defaultConfig } of providerCatalog) {
      expect(defaultProviderConfigs[meta.name]).toBe(defaultConfig);
...
- Modify: `tests/extract/config-video.test.ts`
...
Remove the `resolveApiKey` import. This preserves explicit keys, env indirection, shell commands, and fallback env behavior because those paths still run through `ConfigManager` and `resolveProviderKey` at their existing boundaries.

- [ ] **Step 3: Prove defaults and fallback behavior did not drift.**

Keep the existing `FALLBACK_ENV_MAP` assertions and add a set comparison in `tests/config.test.ts` between `Object.keys(loadMergedConfig(...).providers)` and the catalog provider names. Retain the Gemini assertions in `tests/extract/config-video.test.ts`.

Run:

```bash
pnpm exec vitest run tests/providers/catalog.test.ts tests/providers/all.test.ts tests/config.test.ts tests/extract/config-video.test.ts
```
...
- [ ] **Step 2: Build dashboard identity from `providerCatalog`.**

In `src/index.ts`, import `providerCatalog` alongside `allProviders` and derive the tier map and names from the catalog entries:

```ts
const tierMap = new Map(providerCatalog.map(({ meta }) => [meta.name, meta.tier]));
const allProviderNames = providerCatalog.map(({ meta }) => meta.name);
```

Keep `allProviders` for the `ConfigManager` constructor until phase 3 changes the operation-policy boundary. The dashboard must continue showing all built-ins, including disabled providers.

- [ ] **Step 3: Add the SearXNG injection regression test.**

In `tests/config-manager.test.ts`, instantiate the real `searxng` metadata with a config key and spy on the resulting provider request headers. Assert that the resolved key is used without importing `config.ts` from the provider module. Keep the test deterministic by using the existing mocked `resolveApiKey`.
...
```bash
pnpm exec vitest run tests/providers/catalog.test.ts tests/providers/all.test.ts tests/config.test.ts tests/config-manager.test.ts tests/extract/config-video.test.ts tests/commands/tools.test.ts tests/commands/tools-actions.test.ts tests/commands/tools-dashboard.test.ts
pnpm check
git diff --check
```
...
```bash
git add src/providers/catalog.ts src/providers/all.ts src/providers/searxng.ts src/config.ts src/config-manager.ts src/index.ts tests
